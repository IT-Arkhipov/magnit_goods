...
with sync_playwright() as p:
    ...
    for idx, category in enumerate(root_categories):
        ...
        print(
            f"[{idx + 1}/{len(root_categories)}] Checking: {cat_title} (ID: {cat_id})"
        )
                ...
                if link.count() > 0:
                    ...
                    if cat_id in captured_data:
                        ...
                        if cap["api_title"] != cat_title:
                            ...
                            print(f"  OK")
                        print(f"  Subcategories: {len(cap['subcategories'])}")
                    else:
...
if mismatches:
    ...
    print(f"Updated {len(mismatches)} categories")
else:
    print("All categories are up to date")
