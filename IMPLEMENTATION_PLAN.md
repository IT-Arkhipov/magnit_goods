...
CREATE TABLE stores (
    ...
    store_code  TEXT    NOT NULL UNIQUE,  -- код магазина из API Магнита
    store_type  TEXT    NOT NULL,         -- Магнит, М.Косметик, Мини, Экстра...
...
| `POST` | `/api/stores/scan` | **Сканирование магазинов по городу/улице через API Магнита** |
| `POST` | `/api/stores/scan-by-codes` | Сканирование по списку кодов магазинов |
| `POST` | `/api/stores/select` | Выбор магазина из БД по адресу и типу |
| `POST` | `/api/stores/delete-batch` | Массовое удаление по списку ID |
...
### `POST /api/stores/scan` — сканирование магазинов через API
...
**Прогресс через `GET /api/jobs/42`:**
```json
{
  "job_id": 42,
  "status": "completed",
  "progress": 100,
  "progress_message": "Завершено: найдено 10, добавлено 10, обновлено 0",
  "items_scanned": 10,
  "items_added": 10,
  "items_updated": 0
}
...
Проверено через Playwright на сайте magnit.ru:

| API код | UI-лейбл | Описание |
|---------|----------|----------|
| `MM` | Магнит | Стандартный магазин |
| `ME` | Мини | Магнит Мини |
| `DG` | М.Косметик | Магнит Косметик (фильтр «М.Косметик» на сайте → API код DG) |
| `GM` | Экстра | Магнит Экстра |
| `MO` | Опт | Магнит Опт |
...
### Сценарий 1: Сканирование магазинов через API Магнита

```
Пользователь → Вводит: город, улица, типы магазинов → POST /api/stores/scan

1. Сервер принимает запрос, создаёт ScanJob (status='pending')
2. Возвращает job_id клиенту сразу
3. Фоновая задача:
   ├── Создаёт StoresAPI клиент
   ├── POST /webgate/v1/stores-facade/search/detail с пагинацией
   ├── Парсит ответ: store_code, store_type, full_address, city
   ├── Для каждого магазина: INSERT (новый) или UPDATE (если force_update)
   └── Обновляет ScanJob: status='completed', items_scanned, items_added

4. Клиент: GET /api/jobs/{job_id} каждые 2 сек → прогресс-бар
```
...
2. **Сканирование по адресу:**
   ...
   - Чекбоксы типов (по умолчанию: Магнит + Экстра)
   - Кнопка «Сканировать» → фоновая задача с прогресс-баром

3. **Сканирование по кодам:**
   - Поле «Коды магазинов» (через запятую)
   - Dropdown «Тип» (опционально)
   - Кнопка «Сканировать»

4. **Таблица магазинов:**
   - Чекбокс «Выбрать все» в заголовке
   - Чекбоксы у каждой строки
   - Колонки: ID, Тип, Город, Адрес, Код
...
- Сканирование через API Магнита (`POST /api/stores/scan`)
