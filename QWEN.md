...
| ORM-модели | `Store`, `Category`, `Product`, `PriceHistory`, `ScanJob` |
| API Магнита | `requests` (прямые HTTP-запросы к `webgate/v1/stores-facade/search`) |
| UI | Jinja2 шаблоны + HTMX |
...
| Метод | Путь | Описание |
|-------|------|----------|
| `GET` | `/api/stores` | Список всех магазинов (фильтры: `?city=...&store_type=...`) |
| `POST` | `/api/stores` | Добавить магазин вручную |
| `GET` | `/api/stores/search?q=...` | Поиск по адресу |
| `POST` | `/api/stores/scan` | Сканировать магазины по городу/улице через API Магнита |
...
Маппинг API-кодов в UI-лейблы (проверено через Playwright на magnit.ru):
...
- Модули каталога и мониторинга цен — в разработке (см. план)
